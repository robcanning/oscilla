...
function makeEditor({ x, y, initialText = "" }) {
  ...
  return { wrap, ta, scopeChk, btnSave, btnCancel };
}
...
function makePinEl(annotation, onClick) {
  ...
  dot.style.background = annotation.style?.color || "rgba(255,255,255,0.9)";
  dot.style.boxShadow = "0 6px 16px rgba(0,0,0,0.35)";
  dot.style.border = "1px solid rgba(0,0,0,0.25)";
  pin.appendChild(dot);

  const label = document.createElement("div");
  label.textContent = annotation.text.length > 40
...
function closeEditor() {
  if (state.activeEditor?.wrap) state.activeEditor.wrap.remove();
  state.activeEditor = null;
}

function openEditorAt({ screenX, screenY, initialText, onSave }) {
  closeEditor();

  const editor = makeEditor({ x: screenX, y: screenY, initialText });
  document.body.appendChild(editor.wrap);
  editor.ta.focus();

  editor.btnCancel.onclick = () => closeEditor();
...
function openEditForExisting(annotation) {
  // Minimal: allow delete; edit later if desired
...
function attachDomLayersIfPossible() {
  // score layer
  const score = getScoreContainer();
  if (score && !state.scoreLayer) {
    ensureRelativeContainer(score);
    state.scoreLayer = ensureLayer(score, "oscilla-annotations-layer-score");
  }

  // page layer
  const content = getPageContentContainer();
  if (content && !state.pageLayer) {
    ensureRelativeContainer(content);
    state.pageLayer = ensureLayer(content, "oscilla-annotations-layer-page");
  }
}
...
export function setAnnotationMode(on) {
  ...
  // Optional: show a subtle cursor change
