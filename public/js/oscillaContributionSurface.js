 ...
 */
function showRecordingModal(annotationId) {
 ...
 */
async function uploadRecordedAudio(blob, mimeType, annotationId, sourceInput, statusMsg) {
    const projectName = getProjectName();
    if (!projectName) {
        statusMsg.textContent = "✗ No project loaded";
        return null;
    }

    // Generate filename: ann_<id>_<timestamp>.<ext>
    ...
    try {
        ...
        let url = `/api/upload-audio/${projectName}?subdir=${encodeURIComponent(subdir)}`;
