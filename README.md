...
* `cue_pause` – temporarily pauses playback. If the pause duration is greater than 3 seconds (e.g. `cue_pause_30`), a visual countdown timer will appear to indicate the wait time.
* `cue_stop` – stops playback entirely.
* `cue_repeat_s(...)_e(...)_x(...)_r(...)` – repeat between two cue IDs for a set count.
* `cue_traverse_o(...)_p(...)` – move an object between SVG points.
* `cue_audio`, `cue_video` – media playback triggers.
* `cue_anime` – triggers a fullscreen SVG-based animation overlay. The animation plays for a defined duration, set using `cue_anime_dur(90)` (in seconds). This cue is ideal for timed transitions, gestures, or expressive visual moments. It can also be combined with `cue_choice` and `_next(...)` to create page-based hypermedia scores ("hyperscores"). See full documentation for usage details.
* `cue_choice` – interactive or branching cue logic (e.g., conditional actions or user selection).
* `_next(...)` – chain cues together for branching logic.
